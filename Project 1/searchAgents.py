...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    heuristic, position, cornersVisited = 0, state[0], state[1]
    for corner in corners:
        if not cornersVisited[corners.index(corner)]:
            heuristic = max(heuristic, abs(position[0] - corner[0]) + abs(position[1] - corner[1]))
    return heuristic
...
def foodHeuristic(state, problem):
    ...
    #dist = [manhattanDistance(food_position, position) for food_position in foodGrid.asList()]    
